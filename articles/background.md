...
to format numbers in decimal format, as percentages or currencies, which
could also be passed to the `labels` argument of `ggplot2`s family of
...
practice formatting numbers and styling axes quickly becomes cumbersome
and annoying as you always have to
...
- set the decimal and grouping marks (if deviating from the defaults) so
  ...
  to format a number `x` in decimal format according to style
  conventions found in many European countries.
...
[`countryscales::label_number_de`](https://trekonom.github.io/countryscales/reference/label-de.md):
...
![](background_files/figure-html/style-scales-1.png)

As this simple example shows displaying numbers in decimal format or as
percentages requires some typing, especially if you want to or have to
