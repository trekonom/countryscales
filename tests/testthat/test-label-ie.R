...
test_that("returns same formatted strings as Intl.js", {
  expect_equal(
    label_number_ie(accuracy = 1)(c(x, -x)),
    unlist(
  ...
  # directly here rather than label_currency_ie() (which defaults to "EUR")
  expect_equal(
...
test_that("label_currency_ie uses EUR as the default currency", {
  expect_equal(
    label_currency_ie()(c(z, -z)),
    label_currency_locale(locale = "en-IE", currency = "EUR")(c(z, -z))
  )
})
