...
test_that("returns same formatted strings as Intl.js", {
  expect_equal(
    label_number_gb(accuracy = 1)(c(x, -x)),
    unlist(
  ...
  # directly here rather than label_currency_gb() (which defaults to "GBP")
  expect_equal(
...
test_that("label_currency_gb uses GBP as the default currency", {
  expect_equal(
    label_currency_gb()(c(z, -z)),
    label_currency_locale(locale = "en-GB", currency = "GBP")(c(z, -z))
  )
})
