...
test_that("returns same formatted strings as Intl.js", {
  expect_equal(
    label_number_mx(accuracy = 1)(c(x, -x)),
    unlist(
  ...
  # directly here rather than label_currency_mx() (which defaults to "MXN")
  expect_equal(
...
test_that("label_currency_mx uses MXN as the default currency", {
  expect_equal(
    label_currency_mx()(c(z, -z)),
    label_currency_locale(locale = "es-MX", currency = "MXN")(c(z, -z))
  )
})
