...
test_that("returns same formatted strings as Intl.js", {
  expect_equal(
    label_number_fi(accuracy = 1)(c(x, -x)),
    unlist(
  ...
  # directly here rather than label_currency_fi() (which defaults to "EUR")
  expect_equal(
...
test_that("label_currency_fi uses EUR as the default currency", {
  expect_equal(
    label_currency_fi()(c(z, -z)),
    label_currency_locale(locale = "fi-FI", currency = "EUR")(c(z, -z))
  )
})
