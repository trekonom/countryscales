...
test_that("returns same formatted strings as Intl.js", {
  expect_equal(
    label_number_at(accuracy = 1)(c(x, -x)),
    unlist(
  ...
  # directly here rather than label_currency_at() (which defaults to "EUR")
  expect_equal(
...
test_that("label_currency_at uses EUR as the default currency", {
  expect_equal(
    label_currency_at()(c(z, -z)),
    label_currency_locale(locale = "de-AT", currency = "EUR")(c(z, -z))
  )
})
