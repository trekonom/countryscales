...
test_that("returns same formatted strings as Intl.js", {
  expect_equal(
    label_number_ca(accuracy = 1)(c(x, -x)),
    unlist(
  ...
  # directly here rather than label_currency_ca() (which defaults to "CAD")
  expect_equal(
...
test_that("label_currency_ca uses CAD as the default currency", {
  expect_equal(
    label_currency_ca()(c(z, -z)),
    label_currency_locale(locale = "en-CA", currency = "CAD")(c(z, -z))
  )
})
