...
test_that("returns same formatted strings as Intl.js", {
  expect_equal(
    label_number_cn(accuracy = 1)(c(x, -x)),
    unlist(
  ...
  # directly here rather than label_currency_cn() (which defaults to "CNY")
  expect_equal(
...
test_that("label_currency_cn uses CNY as the default currency", {
  expect_equal(
    label_currency_cn()(c(z, -z)),
    label_currency_locale(locale = "zh-Hans-CN", currency = "CNY")(c(z, -z))
  )
})
