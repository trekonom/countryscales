...
test_that("returns same formatted strings as Intl.js", {
  expect_equal(
    label_number_br(accuracy = 1)(c(x, -x)),
    unlist(
  ...
  # directly here rather than label_currency_br() (which defaults to "BRL")
  expect_equal(
...
test_that("label_currency_br uses BRL as the default currency", {
  expect_equal(
    label_currency_br()(c(z, -z)),
    label_currency_locale(locale = "pt-BR", currency = "BRL")(c(z, -z))
  )
})
