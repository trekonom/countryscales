...
test_that("returns same formatted strings as Intl.js", {
  expect_equal(
    label_number_se(accuracy = 1)(c(x, -x)),
    unlist(
  ...
  # directly here rather than label_currency_se() (which defaults to "SEK")
  expect_equal(
...
test_that("label_currency_se uses SEK as the default currency", {
  expect_equal(
    label_currency_se()(c(z, -z)),
    label_currency_locale(locale = "sv-SE", currency = "SEK")(c(z, -z))
  )
})
