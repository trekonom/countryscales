...
test_that("returns same formatted strings as Intl.js", {
  expect_equal(
    label_number_id(accuracy = 1)(c(x, -x)),
    unlist(
  ...
  # directly here rather than label_currency_id() (which defaults to "IDR")
  expect_equal(
...
test_that("label_currency_id uses IDR as the default currency", {
  expect_equal(
    label_currency_id()(c(z, -z)),
    label_currency_locale(locale = "id-ID", currency = "IDR")(c(z, -z))
  )
})
