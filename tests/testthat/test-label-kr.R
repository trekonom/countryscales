...
test_that("returns same formatted strings as Intl.js", {
  expect_equal(
    label_number_kr(accuracy = 1)(c(x, -x)),
    unlist(
  ...
  # directly here rather than label_currency_kr() (which defaults to "KRW")
  expect_equal(
...
test_that("label_currency_kr uses KRW as the default currency", {
  expect_equal(
    label_currency_kr()(c(z, -z)),
    label_currency_locale(locale = "ko-KR", currency = "KRW")(c(z, -z))
  )
})
