...
test_that("returns same formatted strings as Intl.js", {
  expect_equal(
    label_number_no(accuracy = 1)(c(x, -x)),
    unlist(
  ...
  # directly here rather than label_currency_no() (which defaults to "NOK")
  expect_equal(
...
test_that("label_currency_no uses NOK as the default currency", {
  expect_equal(
    label_currency_no()(c(z, -z)),
    label_currency_locale(locale = "nb-NO", currency = "NOK")(c(z, -z))
  )
})
