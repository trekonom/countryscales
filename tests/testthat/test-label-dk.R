...
test_that("returns same formatted strings as Intl.js", {
  expect_equal(
    label_number_dk(accuracy = 1)(c(x, -x)),
    unlist(
  ...
  # directly here rather than label_currency_dk() (which defaults to "DKK")
  expect_equal(
...
test_that("label_currency_dk uses DKK as the default currency", {
  expect_equal(
    label_currency_dk()(c(z, -z)),
    label_currency_locale(locale = "da-DK", currency = "DKK")(c(z, -z))
  )
})
